...
use crate::{distribution_trait::{DiscreteDistribution, Parametric}, domain::DiscreteDomain};
...
impl DiscreteDistribution for Geometric {
    ...
    fn quantile(&self, x: f64) -> f64 {
        if x.is_nan() {
            // x is not valid
            std::panic!("Tried to evaluate the quantile function of Geometric with a NaN value. \n");
        }
